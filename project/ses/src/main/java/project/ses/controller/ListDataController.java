...
// 7.List all Batches that were completed
	...
	public List<Batch> getAllBatchesCompletedTilNow() {
		List<Batch> BatchesList = batchRepo.getAllBatchesCompleted();
		if (BatchesList.isEmpty()) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No currently completed batch students found");
		}
		return BatchesList;
	}
...
//8.List all Students of currently running batches
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Listing students in running batches"),
			@ApiResponse(responseCode = "500", description = "Internal server error") })
	public List<Student> getAllStudentsPresentRunning() {
		List<Student> studentsCurrently = studentRepo.getAllStudentsCurrentlyRunning();
		if (studentsCurrently.isEmpty()) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No currently running batch students found.");
		}
		return studentsCurrently;
	}
...
//9.List all Students for a particular course
	...
	public List<Student> getAllStudentsByCourse(@PathVariable("code") String code) {
		...
		if (students.isEmpty()) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No students found for the given code.");
		}
		return students;
	}
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Listing students for the batch"),
			@ApiResponse(responseCode = "500", description = "Internal server error") })
	public List<Student> listAllStudentsByBatchId(@PathVariable("batchid") int batchId) {
		List<Student> studentList = studentRepo.getAllStudentByBatchId(batchId);
		if (studentList.isEmpty()) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No students found for the given batchid.");
		}
		return studentList;
	}

// 11.List all batches of a particular course
	@GetMapping("/Batches/ByCourse/{code}")
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Listing batches for the course"),
			@ApiResponse(responseCode = "404", description = "Given code doesn't exist"),
			@ApiResponse(responseCode = "500", description = "Internal server error") })
	public List<Batch> listAllBatchesByCourseCode(@PathVariable("code") String Code) {
		List<Batch> batchesList = batchRepo.getAllBatchesByCourseCode(Code);
		if (batchesList.isEmpty()) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No batches found for the given code.");
		}
		return batchesList;
	}
